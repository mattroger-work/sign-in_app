...
exports.convert_to_pdf = function(date, callback){
    sharp.cache({files:0});
    sharp.cache(false);
...
exports.convert_email = async function(first_name, last_name, tek_email){
    html = fs.readFileSync('./visitor_tag/tag.html', 'utf8');

    
    img = await node_html_to_image({
        html: html
    });
    
    data = await sharp(img)
        .extract({width:190, height:335, left: 0, top:0})
        .toBuffer((err, data, info) =>{
            data = data.toString('base64')
            data = "data:image/png;base64," + data
            ps_con.send_mail(first_name, last_name, tek_email, data)
      })
...
exports.create_pdf = async function(first_name, last_name, photo, escort_req, class_visit, poi){
    ...
    fs.writeFileSync('./visitor_tag/tag.html', html);
